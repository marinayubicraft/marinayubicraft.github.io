...
window.addEventListener("resize", function (e) {
	...
	if (document.documentElement.clientWidth > w && viewportWidth <= w) {
		entriesListShown = true
		wrapper.classList.remove("listHidden")
	}
...
function renderBackground(atlas) {
	...
	backgroundContext.fillStyle = "rgba(0, 0, 0, 0.2)"
...
function buildObjectsList(filter = null, sort = null) {
	...
	if (!sortedAtlas) {
		sortedAtlas = atlas.concat()
		document.getElementById("atlasSize").innerHTML = "The map contains " + sortedAtlas.length + " entries."
	}
	...
	for (let i = entriesOffset; i < entriesOffset + entriesLimit; i++) {
		...
		element.addEventListener("mouseenter", function (e) {
			if (!fixed && !dragging) {
				...
				scaleZoomOrigin = [
					w / 2 - this.entry.center[0]
					, h / 2 - this.entry.center[1]
				]
...
function highlightEntryFromUrl() {
	...
	if (id) {
		...
		if (entries.length === 1) {
			...
			scaleZoomOrigin = [
				w / 2 - entry.center[0]// + container.offsetLeft
				, h / 2 - entry.center[1]// + container.offsetTop
			]
